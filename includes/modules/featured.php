 ...
 */
if (!defined('FEATURED_LIMIT')) define('FEATURED_LIMIT', '8');
if (!defined('MOBILE_FEATURED_LIMIT')) define('MOBILE_FEATURED_LIMIT', '8');
if (!defined('FEATURED_PER_ROW')) define('FEATURED_PER_ROW', '4');
if (!defined('FEATURED_IDS')) define('FEATURED_IDS', '');
if (!defined('FEATURED_SKUS')) define('FEATURED_SKUS', '');

// 是否是M端
if (true == $_isMobileTemplate) {
	$pageLimit = MOBILE_FEATURED_LIMIT;
} else {
	$pageLimit = FEATURED_LIMIT;
}
...
if ((FEATURED_IDS!='' || FEATURED_SKUS!='')
	...
	// 拼接
	foreach ($featuredList as $key => $val) {
		$featuredList[$key]['review'] = array(
			'average' => '0',
			'total'   => '0',
			'rating'  => '0'
		);
		if (isset($_reviewList[$val['product_id']])) {
			$featuredList[$key]['review'] = $_reviewList[$val['product_id']];
		}
	}
}
