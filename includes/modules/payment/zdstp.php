...
class zdstp
{
	function process($payment)
	{
		...
		if ($result['error'] == true) {
			...
			$url = $result['order']['postUrl'];
			unset($result['order']['postUrl']);
			if ($mobileDetect->isMobile()) {
				$result['order']['isMobile'] = '1';
			}

			$payment_form  = '<body onLoad="AutoSubmitForm();">' . "\n";
			$payment_form .= '<form method="post" action="' . $url . '" id="downloadForm" name="downloadForm" target="_top">' . "\n";
			foreach ($result['order'] as $key => $val) {
				$payment_form .= '<input type="hidden" value="' . $val . '" name="' . $key . '">' . "\n";
			}
			$payment_form .= '</form>' . "\n";
			$payment_form .= '<script type="text/javascript">' . "\n";
			$payment_form .= 'function AutoSubmitForm(){' . "\n";
			$payment_form .= 'document.downloadForm.submit();' . "\n";
			$payment_form .= '}' . "\n";
			$payment_form .= '</script>' . "\n";
			$payment_form .= '</body>';
