...
function get_shipping_method($shipping_method_id, $product_qty, $product_amount)
{
	...
	if ($result->RecordCount() > 0) {
		if (($product_qty > $result->fields['free_shipping_qty']
			...
			// 获取超过件数费用
			$fee = get_shipping_sub_fee($fee, $result->fields['sub_fee'], $product_qty);
 ...
 * 获取超过件数费用
 */
function get_shipping_sub_fee($fee, $sub_fee, $qty)
{
	if (isset($sub_fee) && strlen($sub_fee) > 0) {
		$subFeeList = json_decode($sub_fee, true);
		if (!empty($subFeeList)) {
			$temList = array();
			foreach ($subFeeList as $key => $val) {
				$temList[$val['num']] = $val['price'];
			}
			// 排序
			krsort($temList);
			foreach ($temList as $key => $val) {
				if ($qty >= $key && is_numeric($val) && $val >= 0) {
					$fee = $val;
					break;
				}
			}
