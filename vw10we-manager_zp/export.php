...
switch ($action) {
	...
	case 'product':
		...
		$sql = "SELECT p.sku, p.name, p.short_description, p.description, p.image, p.url, p.price, p.specials_price,
		...
		$result = $db->Execute($sql);
		while (!$result->EOF) {
			$data .= '"' . $result->fields['sku'] . '","' . $result->fields['category_sku'] . '","' . str_replace('"', '""', $result->fields['name']) . '","' . $result->fields['image'] . '","' . $result->fields['price'] . '","' . $result->fields['specials_price'] . '","","' . $result->fields['sort_order'] . '","' . $result->fields['status'] . '","' . $result->fields['meta_title'] . '","' . $result->fields['meta_keywords'] . '","' . str_replace('"', '""', $result->fields['meta_description']) . '","' . $result->fields['url'] . '","' . str_replace('"', '""', $result->fields['description']) . '","' . str_replace('"', '""', $result->fields['short_description']) . '","' . str_replace('"', '""', $result->fields['filter_1']) . '","' . str_replace('"', '""',$result->fields['filter_2']) . '","' . str_replace('"', '""', $result->fields['filter_3']) . '","' . str_replace('"', '""', $result->fields['filter_4']) . '","' . str_replace('"', '""', $result->fields['filter_5']) . '","' . str_replace('"', '""', $result->fields['filter_6']) . '","' . str_replace('"', '""', $result->fields['filter_7']) . '","' . str_replace('"', '""', $result->fields['filter_8']) . '","' . str_replace('"', '""', $result->fields['filter_9']) . '"' . "\n";
			$result->MoveNext();
		}
	break;
	case 'product_old':
